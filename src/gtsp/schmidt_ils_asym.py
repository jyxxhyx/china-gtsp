...
class GTSP_ILS:
    def __init__(self, dist, clusters, asymmetric=False):
    ...
    def two_opt(self, tour):
        ...
        注意: 反转段在有向距离下不保序——非对称模式由VND跳过。"""
        if getattr(self, 'asymmetric', False):
            return False
        n = len(tour)
        for i in range(n - 1):
...
def load_china():
    import pandas as pd
    df = pd.read_csv('/Users/yixiao/projects/china_gtsp/data/cities.csv')
